document.addEventListener('DOMContentLoaded', function () {
  ...
  markdownButton.addEventListener('click', async function () {
    ...
    try {
      ...
      let response = await MistralFormat.toMarkdown(markdownPrompt.value, {
        model: markdownModel.value,
        options: options
      });

      // Additional cleaning: remove all text and chars before **
      const doubleAsteriskIndex = response.indexOf('**');
      if (doubleAsteriskIndex !== -1) {
        response = response.substring(doubleAsteriskIndex);
      }
