...
function cleanMarkdownResponse(text: string): string {
  ...
  let cleaned = text;
  // Remove all text and characters before the first **
  const doubleAsteriskIndex = cleaned.indexOf('**');
  if (doubleAsteriskIndex !== -1) {
    cleaned = cleaned.substring(doubleAsteriskIndex);
  }
 ...
 */
export async function toMarkdown(prompt: string, options: MarkdownOptions = {}): Promise<string> {
